...
- .best_metric:
  ...
  Default is "rmse". You can choose a metric depending on the model_type
  used. If `regression` then see
...
This uses the
[`parsnip::nearest_neighbor()`](https://parsnip.tidymodels.org/reference/nearest_neighbor.html)
with the `engine` set to `kknn`
